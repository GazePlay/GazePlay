...
import net.gazeplay.games.whereisit.WhereIsItEmmanuel;
 ...
 */
@Slf4j
public class FindTheOddOneOutEmmanuel implements GameLifeCycle {
    ...
    @Getter
    private final FindTheOddOneOutEmmanuelGameType gameType;
    ...
    private RoundDetailsEmmanuel currentRoundDetails;
    ...
    public FindTheOddOneOutEmmanuel(final FindTheOddOneOutEmmanuelGameType gameType, final int nbLines, final int nbColumns, final boolean fourThree,
    ...
    public void launch() {
        if (!this.startGame) {
            ...
            this.startGame = true;
            String rule = "data/common/rules/FindRules.png";
    ...
    private Transition createQuestionTransition(final String question, final List<Image> listOfPictos) {
        ...
        fullAnimation.setOnFinished(actionEvent -> {
            ...
            if (currentRoundDetails != null) {
                ...
                for (final PictureCardEmmanuel p : currentRoundDetails.getPictureCardList()) {
                    //  log.debug("p = {}", p);
                    p.toFront();
                    p.setOpacity(1);
                }
    ...
    void removeAllIncorrectPictureCards() {
        ...
        final List<PictureCardEmmanuel> pictureCardsToHide = new ArrayList<>();
        for (final PictureCardEmmanuel pictureCard : this.currentRoundDetails.getPictureCardList()) {
    ...
    RoundDetailsEmmanuel pickAndBuildRandomPictures(final int numberOfImagesToDisplayPerRound, final ReplayablePseudoRandom random,
        ...
        final GameSizingEmmanuel gameSizing = new GameSizingComputerEmmanuel(nbLines, nbColumns, fourThree)
            .computeGameSizing(gameContext.getGamePanelDimensionProvider().getDimension2D());

        final List<PictureCardEmmanuel> pictureCardList = new ArrayList<>();
        ...
        for (int i = 0; i < numberOfImagesToDisplayPerRound; i++) {
            ...
            final PictureCardEmmanuel pictureCard = new PictureCardEmmanuel(gameSizing.width * posX + gameSizing.shift,
                ...
                randomImageFile + "", stats, this, gamesRules);
        ...
        return new RoundDetailsEmmanuel(pictureCardList, winnerImageIndexAmongDisplayedImages, questionSoundPath, question, pictograms);
    }
    ...
    private String getPathSound(final String folder, final String language) {
        ...
        if (gameType.toString().contains("SOUNDS")) {
            return "data/" + this.gameType.getResourcesDirectoryName() + "/sound/" + folder + ".mp3";
        }
        return "data/" + this.gameType.getResourcesDirectoryName() + "/sound/fra/odd" + "." + voice
            + ".fra.mp3";
    }
