...
public class GameSizingComputer {
    ...
    public net.gazeplay.games.gazeplayEvalTest.GameSizing computeGameSizing(Scene scene) {
        Rectangle2D bounds = new Rectangle2D(0, 0, scene.getWidth(), scene.getHeight());

        return computeGameSizing(bounds);
    }

    public net.gazeplay.games.gazeplayEvalTest.GameSizing computeGameSizing(Pane pane) {
        Rectangle2D bounds = new Rectangle2D(0, 0, pane.getWidth(), pane.getHeight());

        return computeGameSizing(bounds);
    }

    public net.gazeplay.games.gazeplayEvalTest.GameSizing computeGameSizing(Dimension2D dimension2D) {
        Rectangle2D bounds = new Rectangle2D(0, 0, dimension2D.getWidth(), dimension2D.getHeight() * 0.9);

        return computeGameSizing(bounds);
    }

    public net.gazeplay.games.gazeplayEvalTest.GameSizing computeGameSizing(Rectangle2D bounds) {
        ...
        net.gazeplay.games.gazeplayEvalTest.GameSizing gameSizing = new net.gazeplay.games.gazeplayEvalTest.GameSizing(width / nbColumns, height / nbLines, shift);
