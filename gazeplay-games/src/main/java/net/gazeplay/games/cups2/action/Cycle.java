...
public class Cycle implements Action {
    ...
    public void execute(Callback<Void, Void> onFinish) {
        ...
        PathTransition pt;
        if (direction)
            pt = Action.smoothArcTransition(Config.ACTION_CYCLE_TIME, cups.get(start), cups.get(end), upOrDown);
        else
            pt = Action.smoothArcTransition(Config.ACTION_CYCLE_TIME, cups.get(end), cups.get(start), upOrDown);
        pt.setOnFinished(e -> joinCallback.call(null));
        pt.play();

        int dir = direction ? 1 : 0;
        for (Cup cup : cups.subList(start + dir, end + dir)) {
            TranslateTransition tt = new TranslateTransition(Duration.millis(pt.getDuration().toMillis()), cup);
            tt.setByX(distance);
