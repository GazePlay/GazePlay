...
public class DefaultGamesLocator implements GamesLocator {
    ...
    public List<GameSpec> listGames() {
        ...
        result.add(new GameSpec(multilinguism.getTrad("MagicCards", language) + " (2x2)", (gameSpec, scene, root, cbxGames) -> {
            MagicCardsGamesStats stats = new MagicCardsGamesStats(scene);
            Card.addCards(root, scene, cbxGames, 2, 2, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("MagicCards", language) +" (2x3)", (gameSpec, scene, root, cbxGames) -> {
            MagicCardsGamesStats stats = new MagicCardsGamesStats(scene);
            Card.addCards(root, scene, cbxGames, 2, 3, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("MagicCards", language) + " (3x2)", (gameSpec, scene, root, cbxGames) -> {
            MagicCardsGamesStats stats = new MagicCardsGamesStats(scene);
            Card.addCards(root, scene, cbxGames, 3, 2, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("MagicCards", language) + " (3x3)", (gameSpec, scene, root, cbxGames) -> {
            MagicCardsGamesStats stats = new MagicCardsGamesStats(scene);
            Card.addCards(root, scene, cbxGames, 3, 3, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("Blocks", language) + " (2x2)", (gameSpec, scene, root, cbxGames) -> {
            BlocsGamesStats stats = new BlocsGamesStats(scene);
            Blocs.makeBlocks(scene, root, cbxGames, 2, 2, true, 1, false, stats);
            return stats;
        }));

        result.add(new GameSpec( multilinguism.getTrad("Blocks", language) + " (2x3)", (gameSpec, scene, root, cbxGames) -> {
            BlocsGamesStats stats = new BlocsGamesStats(scene);
            Blocs.makeBlocks(scene, root, cbxGames, 2, 3, true, 1, false, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("Blocks", language) + " (3x3)", (gameSpec, scene, root, cbxGames) -> {
            BlocsGamesStats stats = new BlocsGamesStats(scene);
        ...
        result.add(new GameSpec(multilinguism.getTrad("ColoredBubbles", language), (gameSpec, scene, root, cbxGames) -> {
            BubblesGamesStats stats = new BubblesGamesStats(scene);
            Bubble bubble = new Bubble(scene, root, Bubble.COLOR, stats, true);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("PortraitBubbles", language), (gameSpec, scene, root, cbxGames) -> {
            BubblesGamesStats stats = new BubblesGamesStats(scene);
            Bubble bubble = new Bubble(scene, root, Bubble.PORTRAIT, stats, false);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("WhereIsTheAnimal", language) + " (2x2)", (gameSpec, scene, root, cbxGames) -> {
        ...
        result.add(new GameSpec(multilinguism.getTrad("WhereIsTheAnimal", language) + " (2x3)", (gameSpec, scene, root, cbxGames) -> {
            WhereIsItStats stats = new WhereIsItStats(scene);
            WhereIsIt.buildGame(WhereIsIt.ANIMALNAME, 2, 3, false, root, scene, cbxGames, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("WhereIsTheAnimal", language) + " (3x2)", (gameSpec, scene, root, cbxGames) -> {
            WhereIsItStats stats = new WhereIsItStats(scene);
            WhereIsIt.buildGame(WhereIsIt.ANIMALNAME, 3, 2, true, root, scene, cbxGames, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("WhereIsTheAnimal", language) + " (3x3)", (gameSpec, scene, root, cbxGames) -> {
            WhereIsItStats stats = new WhereIsItStats(scene);
            WhereIsIt.buildGame(WhereIsIt.ANIMALNAME, 3, 3, false, root, scene, cbxGames, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("WhereIsTheColor", language) + " (2x2)", (gameSpec, scene, root, cbxGames) -> {
            WhereIsItStats stats = new WhereIsItStats(scene);
            WhereIsIt.buildGame(WhereIsIt.COLORNAME, 2, 2, false, root, scene, cbxGames, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("WhereIsTheColor", language) + " (2x3)", (gameSpec, scene, root, cbxGames) -> {
            WhereIsItStats stats = new WhereIsItStats(scene);
            WhereIsIt.buildGame(WhereIsIt.COLORNAME, 2, 3, false, root, scene, cbxGames, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("WhereIsTheColor", language) + " (3x2)", (gameSpec, scene, root, cbxGames) -> {
            WhereIsItStats stats = new WhereIsItStats(scene);
            WhereIsIt.buildGame(WhereIsIt.COLORNAME, 3, 2, false, root, scene, cbxGames, stats);
            return stats;
        }));

        result.add(new GameSpec(multilinguism.getTrad("WhereIsTheColor", language) + " (3x3)", (gameSpec, scene, root, cbxGames) -> {
            WhereIsItStats stats = new WhereIsItStats(scene);
